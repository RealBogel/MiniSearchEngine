...
public class SearchClient {
    ...
    public static final int MIN_NUM_RATINGS = 2;
    ...
    // Behavior:
    //   - This method creates an inverted index for a list of documents, using the document words
    //     in a case insensitive manner.
    // Parameters:
    //   - docs: a List of Media "documents".
    // Returns: a Map where documents that contain the same word are grouped together.
    // Exceptions:
    public static Map<String, Set<Media>> createIndex(List<Media> docs) {
        Map<String, Set<Media>> map = new TreeMap<>(); 
        for(Media media: docs) {
            List<String> curr = media.getContent();
            for(String token: curr) {
                String lowerToken = token.toLowerCase();
                if(!map.keySet().contains(lowerToken)) {
                    map.put(lowerToken, new HashSet<>());
                } 
                map.get(lowerToken).add(media);
            }
        }
        return map;
    }
    ...
    // Behavior:
    //   - This method creates an inverted index for a list of documents.
    // Parameters:
    //   - index: an inverted index Map of terms to the Set of media containing those terms.
    //   - query: a String of the given search query.
    // Returns: a Set of media from the index that contain the given search query, ranking each media 
    //          from most to least ratings.
    // Exceptions:
